 ...
 */
export function probeLoaderHealth(loader: LoaderLike | undefined, moduleName: string): HealthStatus {
  ...
  try {
    ...
    if (matches.length === 0 || matches.every(entry => entry.disabled === true)) return 'unavailable'
    const states = matches
      .filter(entry => entry.disabled !== true)
      .map(entry => entry.fiber?.state)
    if (states.includes(FIBER_PHASE.ACTIVE)) return 'ok'
    if (states.includes(FIBER_PHASE.FAILED)) return 'error'
    if (states.some(state => state === FIBER_PHASE.PENDING
      || state === FIBER_PHASE.LOADING
      || state === FIBER_PHASE.DISPOSED
      || state === FIBER_PHASE.UNLOADING)) return 'unavailable'
    return 'unknown'
  } catch {
    return 'unknown'
  }
...
export function healthText(status: HealthStatus): string {
  if (status === 'ok') return '当前运行 OK'
  if (status === 'unavailable') return '当前暂不可用'
  if (status === 'error') return '当前运行异常'
  return '暂时无法判断'
}
