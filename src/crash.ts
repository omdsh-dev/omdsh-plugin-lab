...
export function sanitizeCrashFrame(stack: unknown): string | undefined {
  if (typeof stack !== 'string') return undefined
  for (const line of stack.split('\n').slice(1)) {
    const matched = line.match(LOCATION)?.[0]
    if (matched === undefined) continue
    const normalized = matched.replace(/^file:\/\//u, '').replaceAll('\\', '/')
    if (normalized.startsWith('node:')) return normalized.slice(0, 200)
    const dependency = normalized.lastIndexOf('/node_modules/')
    if (dependency >= 0) return `node_modules/${normalized.slice(dependency + '/node_modules/'.length)}`.slice(0, 200)
...
export function runtimeCrashSignal(
  error: unknown,
  origin: RuntimeCrashSignal['origin'],
): RuntimeCrashSignal {
  const row = typeof error === 'object' && error !== null ? error as { name?: unknown; stack?: unknown } : {}
