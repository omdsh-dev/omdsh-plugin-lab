...
function parseReceipt(eventId, value) {
    ...
    const status = statuses.has(row.status) ? row.status : undefined;
...
export class ExperienceUploader {
    ...
    async flushPending(targetEventId) {
        const delivered = new Map();
        for (const record of this.store.pending()) {
            if (targetEventId !== undefined && record.event.eventId !== targetEventId)
                continue;
    ...
    async refreshReceipts() {
        ...
        for (const previous of this.store.latestReceipts()) {
            if (previous.followToken === undefined || previous.updatesUrl === undefined)
                continue;
